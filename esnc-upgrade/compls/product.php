...
function productlinklist($id,$top,$ctrl=PDLINK_CTRL_LINK,$type=NULL,$fields='`a`.`id`,`a`.`name`,`a`.`ctrl`,`a`.`type`,`a`.`img1`,`a`.`summary`,`a`.`urlrewrite`, `c`.`urlrewrite` caturlrewrite'){
	_trace(__FUNCTION__);
	global $sql;
	settype($id,'int');
	settype($ctrl,'int');
	settype($top,'int');
	$sql = 'SELECT DISTINCT '.$fields.' FROM `'.DB_TABLE_PREFIX.'product` as `a` INNER JOIN `'.DB_TABLE_PREFIX.'productlink` as `b` ON `a`.`id`=`b`.`linkID` INNER JOIN `'.DB_TABLE_PREFIX.'catproductproduct` as `d` ON `a`.`id` = `d`.`productid`  INNER JOIN `'.DB_TABLE_PREFIX.'catproduct` as `c` ON `d`.`catproductid` = `c`.`id`  WHERE `b`.`productID`='.$id.' LIMIT '.$top;
	_trace($sql);
	return mysql_query($sql);
}
