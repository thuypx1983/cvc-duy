<?php
require_once("esnc-upgrade/class/mail/class.phpmailer.php");
class mailer{
	...
	function send($alert=0,$proto=0){
		...
		$this->from='"'.($this->sendername ? $this->sendername : $this->from).'"<'.$sendmail_from.'>';
       ...
       // $this->header=strtr($mail_header,$tag);
		...
		$mailer->IsSMTP();
		$mailer->Host ='whm27.smartseohosting.net';
		$mailer->SMTPAuth = true;
		$mailer->SMTPSecure = "tls";   
		$mailer->Port       = 587; 
		$mailer->Username = 'webmail@circuit-vietnam-cambodge.com';
		$mailer->Password = 'uf5l~6f7o~#U';

		if($this->sendername) $mailer->FromName = $this->sendername;
		else $mailer->FromName = T_SENDNAME;
		$mailer->From = EMAIL_SALES;
		$re = explode(',',$this->recipient);
		foreach($re as $e){
			$mailer->AddAddress($e);
		}
